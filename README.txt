...
An example of a synchronization error would be when there are multiple operations that need to be performed.
For example "2*(4*3)" because we dont have synchronization, the degrouper could go first leaving the following
in the buffer "2+4*3" and then the system would do a random process becasue there is nos specification of what to do.
The program could add 2+4 first having 6 and then multiplying to make 18 or it could multiply 4*3 which is 12 and then
add 2 which would be 14. 2 completely different answers.
...
No it should not generate incorrect output becasue it is supposed to have correct synchronization which avoids this.
The only way I see it getting an incorrect output would be if the input was wrong like if the y input (2+3*5), then there would
be a problem with the output. 
...
Why?
The specific data structures would be the buffer array that contains the values that need to be changed
also, in the case of the addition and multiplying it'd be the temporary array created to hold the result.
Thay are protected because we wouldnt want them to be changed at the same time that another operation is working.


Q4: What would happen if you had a busy-wait within one of your critical
sections?  What if it is a loop with sched_yield()?
If I had a busy wait then it would keep on executing throughout the critical section just waiting on something that might have not happened yet.
If it has the sched_yield() in it then it would stop running that specific thread and move it back to the queue, letting other threads move on and 
this would prevent busy-wait scenario.
...
No it is not required because it is part of the program itself, it is between the mutex
of the previous implementation. When the operation enters the mutex area it locks and is
able to do the specific type of operation and update num_ops, then it unlocks it. So no,
a seperate one is not required.
...
It is important to keep them small becasue there are other threads waiting to be locked themselves.
Being that there is a shared resource, the threads share it and it helps in the time that they are idle by
having small critical sections.
